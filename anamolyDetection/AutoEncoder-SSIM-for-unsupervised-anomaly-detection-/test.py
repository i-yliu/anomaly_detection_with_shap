...
from utils import read_img, get_patch, patch2img, set_img_color, bg_mask, mask_image
...
def get_residual_map(img_path, cfg):
    ...
    seg = np.zeros(test_img_.shape[:2])
    ...
    if test_img.shape[:2] == (cfg.patch_size, cfg.patch_size):
        ...
        def f(z):
            ...
            decoded_img = autoencoder.predict(mask_image(z, seg_slice, img_orig, 1),0)

            rec_img = np.reshape((decoded_img * 255.).astype('uint8'), test_img.shape)
            l1_residual_map = np.abs(test_img / 255. - rec_img / 255.)
            import pdb; pdb.set_trace()
            return np.sum(l1_residual_map)

        import pdb; pdb.set_trace()
        explainer = shap.KernelExplainer(f, np.zeros((1,64)))
        shap_values = explainer.shap_values(np.ones((1,64)), nsamples='auto') # runs VGG16 1000 times
        ##
